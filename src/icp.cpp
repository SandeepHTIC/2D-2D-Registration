...
cv::Mat plate12withICP_noInputs(const cv::Mat &blob_image, const cv::Point2f &Centre, const std::string &path_d)
{
    ...
    for (int i = 0; i < N; ++i)
    {
        if (cluster_labels[i] == plate1_id)
            plate1_pts.push_back(centroids[i]);
        else
            plate2_pts.push_back(centroids[i]);
    }
    ...
    if (!plate2_pts.empty())
    {
        ...
        {
            if (db_labels[i] != -1)
                clusters[db_labels[i]].push_back(plate2_pts[i]);
        }
    ...
    if (plate2_dbscan_pts.size() < 12)
    {
        ...
        for (const auto &ap : angle_pts)
            plate2_dbscan_pts.push_back(ap.second);
    }

    // Step 7: Distance filtering (within 40 units of max)
    vector<float> dist_plate2;
    for (auto &pt : plate2_dbscan_pts)
        dist_plate2.push_back(norm(pt - Centre));
    ...
    vector<float> dist1;
    for (auto &pt : plate1_pts)
        dist1.push_back(norm(pt - Centre));
    ...
    for (int i = 0; i < plate1_pts.size(); ++i)
    {
        ...
        if (d >= low && d <= high)
            plate1_final.push_back(plate1_pts[i]);
    }
