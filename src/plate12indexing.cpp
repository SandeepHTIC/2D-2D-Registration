...
static vector<int> convexHullIdx(const vector<Point2d> &pts)
{
    ...
    for (auto &p : pts)
        ptsf.emplace_back((float)p.x, (float)p.y);
    vector<int> idx;
    if (!ptsf.empty())
        convexHull(ptsf, idx, false, false);
    return idx;
}
    ...
    vector<double> &outAng)
{
    ...
    for (int i = 0; i < (int)ang.size(); ++i)
        angWithIdx.emplace_back(ang[i], i);
    sort(angWithIdx.begin(), angWithIdx.end(), [](const auto &a, const auto &b)
         { return a.first < b.first; });

    // Build sorted angle list
    vector<double> sortedAng;
    sortedAng.reserve(angWithIdx.size());
    for (auto &p : angWithIdx)
        sortedAng.push_back(p.first);

    for (int c = 0; c < C; ++c)
    {
        vector<int> look(R);
        for (int r = 0; r < R; ++r)
            look[r] = (int)refAngles[r][c];
    ...
    const Point2d &Centre)
{
    ...
    vector<Point2d> p1;
    safe_reserve(p1, h1.size());
    for (int i : h1)
        p1.push_back(plate1Pts[i]);

    auto h2 = convexHullIdx(plate2Pts);
    vector<Point2d> p2;
    safe_reserve(p2, h2.size());
    for (int i : h2)
        p2.push_back(plate2Pts[i]);
    ...
    {
        if (r[0] >= 240.0)
        { // Match MATLAB threshold exactly
            icpFid.push_back(r);
        }
    }
    if (icpFid.empty())
    ...
    for (int k = 0; k < K; ++k)
    {
        ...
        for (int refCol = 0; refCol < 6; ++refCol)
        {
            ...
            for (int r = 0; r < 6; ++r)
                refAngles[r] = icpOrdAngRef[r][refCol];
    ...
    // Always perform ICP mapping with the best (k, refCol) pair
    {
        ...
        for (int i = 0; i < numIcpFid - 1; ++i)
        {
            ...
            for (int r = 0; r < 6; ++r)
            {
                ...
                if (angle > refAngle - 2 && angle < refAngle + 2)
                { // ±2 tolerance
                    int icpIdx = icpIdxOrd[r][bestRefCol] + 17;
                    icpPlatFid.push_back({static_cast<float>(icpFid[i][1]),
    ...
    if (numIcpFid == 6)
    {
        icpPlatFid.push_back({static_cast<float>(icpFid[5][1]),
