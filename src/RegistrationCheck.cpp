    ...
    try
    {
        ...
        for (int k_idx = k_min; k_idx <= k_max; ++k_idx)
        {
            ...
            if (!matchedDistances.empty())
            {
                ...
                double sum = 0.0;
                size_t cnt = 0;
                for (size_t idx = 0; idx < ksz - drop; ++idx)
                {
                    sum += matchedDistances[idx];
                    ++cnt;
                }
        ...
        if (!matchedDistances.empty())
        {
            ...
            double sum = 0.0;
            size_t cnt = 0;
            for (size_t idx = 0; idx < k - drop; ++idx)
            {
                sum += matchedDistances[idx];
                ++cnt;
            }
            if (cnt > 0)
                hung_err = sum / static_cast<double>(cnt);
        }
        ...
        {
            chosen_err = hung_err;
            chosen_method = "hungarian";
        }
        else if (std::isfinite(best_err))
        {
            chosen_err = best_err;
            chosen_method = "mutualNN";
        }
        else
        {
            chosen_err = 10.0;
