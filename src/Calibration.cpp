...
CalibrationResult Calibration::calibrate(
    ...
    try {
       ...
       // std::string xy_override = path_d + "\\Output\\" + position + "\\PD\\" + position + "_2D_matlab.txt";
        ...
        // Robust label-based matching between detected 2D points and world points
        if (xy_o.cols() < 3) {
        ...
        std::vector<int> labels; labels.reserve(xy_by_label.size());
        for (int l = 1; l <= max_label; ++l) {
            if (xy_by_label.find(l) != xy_by_label.end()) labels.push_back(l);
        }
        ...
        const int s = 6;
        const int max_iters = std::min(5000, 20 * N); // stricter inlier threshold -> more iterations
        const double thresh_px = 0.25; // stricter reprojection threshold
        ...
        std::vector<double> reprojection_errors; reprojection_errors.reserve(N);
        for (int i = 0; i < N; ++i) {
            Eigen::Vector4d Xh(Xw[i].x(), Xw[i].y(), Xw[i].z(), 1.0);
            Eigen::Vector3d p = P0 * Xh;
            double w = p(2);
            if (std::abs(w) < 1e-15) { reprojection_errors.push_back(1e6); continue; }
        ...
        // K already normalized once above (K(3,3)=1)
        ...
        // No ensemble: we directly use P_norm from decomposed and normalized P0
...
std::pair<Eigen::Matrix<double, 3, 4>, std::vector<double>> Calibration::estimateCameraMatrix(
    ...
    // MATLAB-equivalent DLT without RANSAC or non-linear refinement
    // 1) Build homogeneous image points and normalize
    ...
    // 2) Build the A matrix using normalized image coordinates
    ...
    // 3) Solve AtA eigenproblem to match MATLAB eig(A'*A)
    ...
    // 4) Reshape to 4x3 (MATLAB reshape(P,4,3)) and undo normalization
    ...
    if (camMatrix_prime(3,2) < 0) camMatrix_prime = -camMatrix_prime; // match MATLAB camMatrix(end) check

    // Return 3x4 projection matrix
    Eigen::Matrix<double,3,4> P = camMatrix_prime.transpose();

    // 5) Compute reprojection errors on original (unnormalized) image points
